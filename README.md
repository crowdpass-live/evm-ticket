[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/crowdpass-events/ticket)

# CrowdPass Protocol - Ticket Smart Contracts

## Overview

This repository contains the smart contracts for the **Ticket** module of the [CrowdPass Protocol](https://www.crowdpass.events), a decentralized event and ticketing platform. The Ticket contracts manage the lifecycle of event tickets, including creation, sales, fee management, check-in, and administrative controls. The system is modular, upgradeable, and designed for extensibility and security.
...
- `src/inits/CrowdPassInit.sol`: Initialization logic for protocol deployment.
...
- **Fee management**: Multi-fee support per ticket, including CrowdPass platform fees
...
```sh
forge script script/DeployCrowdPassTickets.s.sol --rpc-url <YOUR_RPC_URL> --private-key <YOUR_PRIVATE_KEY> --broadcast
```
...
| Unit | Meaning |
|------|---------|
| `{1/1}` | Fee fraction: `CROWDPASS_FEE_BPS / FEE_BASIS_POINTS` = 300 / 10,000 = 0.03 |

### Precision

- **BPS** -- Basis points (1/10,000). `FEE_BASIS_POINTS = 10_000`, `CROWDPASS_FEE_BPS = 300` (3%).
...
*For more information, visit [crowdpass.events](https://www.crowdpass.events) or contact the maintainers.*
