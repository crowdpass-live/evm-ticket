...
event CrowdPassInitialized(address ticketProxy, FeeType[] feeTypes, address[] tokens);

contract CrowdPassInit {
    ...
    function initCrowdPass(address _admin, address _ticketProxy, FeeType[] calldata _feeTypes, address[] calldata _tokens)
        public
    {
        AccessControlLib.__AccessControl_init(_admin);
        FactoryLib.factoryStorage().ticketProxy = _ticketProxy;
        MarketplaceLib.setFeeTokenAddresses(_feeTypes, _tokens);
        emit CrowdPassInitialized(_ticketProxy, _feeTypes, _tokens);
    }
}
